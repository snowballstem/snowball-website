// Generated by Snowball 2.0.0 - https://snowballstem.org/

/**@constructor*/
function YiddishStemmer() {
    var base = new BaseStemmer();
    /** @const */ var a_0 = [
        ["\u05D0\u05D3\u05D5\u05E8\u05DB", -1, 1],
        ["\u05D0\u05D4\u05D9\u05E0", -1, 1],
        ["\u05D0\u05D4\u05E2\u05E8", -1, 1],
        ["\u05D0\u05D4\u05F2\u05DE", -1, 1],
        ["\u05D0\u05D5\u05DE", -1, 1],
        ["\u05D0\u05D5\u05E0\u05D8\u05E2\u05E8", -1, 1],
        ["\u05D0\u05D9\u05D1\u05E2\u05E8", -1, 1],
        ["\u05D0\u05E0", -1, 1],
        ["\u05D0\u05E0\u05D8", 7, 1],
        ["\u05D0\u05E0\u05D8\u05E7\u05E2\u05D2\u05E0", 8, 1],
        ["\u05D0\u05E0\u05D9\u05D3\u05E2\u05E8", 7, 1],
        ["\u05D0\u05E4", -1, 1],
        ["\u05D0\u05E4\u05D9\u05E8", 11, 1],
        ["\u05D0\u05E7\u05E2\u05D2\u05E0", -1, 1],
        ["\u05D0\u05E8\u05D0\u05E4", -1, 1],
        ["\u05D0\u05E8\u05D5\u05DE", -1, 1],
        ["\u05D0\u05E8\u05D5\u05E0\u05D8\u05E2\u05E8", -1, 1],
        ["\u05D0\u05E8\u05D9\u05D1\u05E2\u05E8", -1, 1],
        ["\u05D0\u05E8\u05F1\u05E1", -1, 1],
        ["\u05D0\u05E8\u05F1\u05E4", -1, 1],
        ["\u05D0\u05E8\u05F2\u05E0", -1, 1],
        ["\u05D0\u05F0\u05E2\u05E7", -1, 1],
        ["\u05D0\u05F1\u05E1", -1, 1],
        ["\u05D0\u05F1\u05E4", -1, 1],
        ["\u05D0\u05F2\u05E0", -1, 1],
        ["\u05D1\u05D0", -1, 1],
        ["\u05D1\u05F2", -1, 1],
        ["\u05D3\u05D5\u05E8\u05DB", -1, 1],
        ["\u05D3\u05E2\u05E8", -1, 1],
        ["\u05DE\u05D9\u05D8", -1, 1],
        ["\u05E0\u05D0\u05DB", -1, 1],
        ["\u05E4\u05D0\u05E8", -1, 1],
        ["\u05E4\u05D0\u05E8\u05D1\u05F2", 31, 1],
        ["\u05E4\u05D0\u05E8\u05F1\u05E1", 31, 1],
        ["\u05E4\u05D5\u05E0\u05D0\u05E0\u05D3\u05E2\u05E8", -1, 1],
        ["\u05E6\u05D5", -1, 1],
        ["\u05E6\u05D5\u05D6\u05D0\u05DE\u05E2\u05E0", 35, 1],
        ["\u05E6\u05D5\u05E0\u05F1\u05E4", 35, 1],
        ["\u05E6\u05D5\u05E8\u05D9\u05E7", 35, 1],
        ["\u05E6\u05E2", -1, 1]
    ];

    /** @const */ var a_1 = [
        ["\u05D3\u05D6\u05E9", -1, -1],
        ["\u05E9\u05D8\u05E8", -1, -1],
        ["\u05E9\u05D8\u05E9", -1, -1],
        ["\u05E9\u05E4\u05E8", -1, -1]
    ];

    /** @const */ var a_2 = [
        ["\u05D5\u05E0\u05D2", -1, 1],
        ["\u05E1\u05D8\u05D5", -1, 1],
        ["\u05D8", -1, 1],
        ["\u05D1\u05E8\u05D0\u05DB\u05D8", 2, 31],
        ["\u05E1\u05D8", 2, 1],
        ["\u05D9\u05E1\u05D8", 4, 1],
        ["\u05D2\u05D9\u05E1\u05D8", 5, 33],
        ["\u05E9\u05D9\u05E1\u05D8", 5, 33],
        ["\u05E9\u05D0\u05E4\u05D8", 2, 1],
        ["\u05D4\u05F2\u05D8", 2, 1],
        ["\u05E7\u05F2\u05D8", 2, 1],
        ["\u05D9\u05E7\u05F2\u05D8", 10, 1],
        ["\u05DC\u05E2\u05DB", -1, 1],
        ["\u05E2\u05DC\u05E2\u05DB", 12, 1],
        ["\u05D9\u05D6\u05DE", -1, 1],
        ["\u05D9\u05DE", -1, 1],
        ["\u05E2\u05DE", -1, 1],
        ["\u05E2\u05E0\u05E2\u05DE", 16, 3],
        ["\u05D8\u05E2\u05E0\u05E2\u05DE", 17, 4],
        ["\u05E0", -1, 1],
        ["\u05E7\u05DC\u05D9\u05D1\u05E0", 19, 14],
        ["\u05E8\u05D9\u05D1\u05E0", 19, 15],
        ["\u05D8\u05E8\u05D9\u05D1\u05E0", 21, 12],
        ["\u05E9\u05E8\u05D9\u05D1\u05E0", 21, 7],
        ["\u05D4\u05F1\u05D1\u05E0", 19, 27],
        ["\u05E9\u05F0\u05D9\u05D2\u05E0", 19, 17],
        ["\u05D6\u05D5\u05E0\u05D2\u05E0", 19, 22],
        ["\u05E9\u05DC\u05D5\u05E0\u05D2\u05E0", 19, 25],
        ["\u05E6\u05F0\u05D5\u05E0\u05D2\u05E0", 19, 24],
        ["\u05D1\u05F1\u05D2\u05E0", 19, 26],
        ["\u05D1\u05D5\u05E0\u05D3\u05E0", 19, 20],
        ["\u05F0\u05D9\u05D6\u05E0", 19, 11],
        ["\u05D8\u05E0", 19, 4],
        ["GE\u05D1\u05D9\u05D8\u05E0", 32, 9],
        ["GE\u05DC\u05D9\u05D8\u05E0", 32, 13],
        ["GE\u05DE\u05D9\u05D8\u05E0", 32, 8],
        ["\u05E9\u05E0\u05D9\u05D8\u05E0", 32, 19],
        ["\u05E1\u05D8\u05E0", 32, 1],
        ["\u05D9\u05E1\u05D8\u05E0", 37, 1],
        ["GE\u05D1\u05D9\u05E1\u05E0", 19, 10],
        ["\u05E9\u05DE\u05D9\u05E1\u05E0", 19, 18],
        ["GE\u05E8\u05D9\u05E1\u05E0", 19, 16],
        ["\u05E2\u05E0", 19, 1],
        ["\u05D2\u05D0\u05E0\u05D2\u05E2\u05E0", 42, 5],
        ["\u05E2\u05DC\u05E2\u05E0", 42, 1],
        ["\u05E0\u05D5\u05DE\u05E2\u05E0", 42, 6],
        ["\u05D9\u05D6\u05DE\u05E2\u05E0", 42, 1],
        ["\u05E9\u05D8\u05D0\u05E0\u05E2\u05E0", 42, 29],
        ["\u05D8\u05E8\u05D5\u05E0\u05E7\u05E0", 19, 23],
        ["\u05E4\u05D0\u05E8\u05DC\u05F1\u05E8\u05E0", 19, 28],
        ["\u05E9\u05F0\u05F1\u05E8\u05E0", 19, 30],
        ["\u05F0\u05D5\u05D8\u05E9\u05E0", 19, 21],
        ["\u05D2\u05F2\u05E0", 19, 5],
        ["\u05E1", -1, 1],
        ["\u05D8\u05E1", 53, 4],
        ["\u05E0\u05E1", 53, 1],
        ["\u05D8\u05E0\u05E1", 55, 4],
        ["\u05E2\u05E0\u05E1", 55, 3],
        ["\u05E2\u05E1", 53, 1],
        ["\u05D9\u05E2\u05E1", 58, 2],
        ["\u05E2\u05DC\u05E2\u05E1", 58, 1],
        ["\u05E2\u05E8\u05E1", 53, 1],
        ["\u05E2\u05E0\u05E2\u05E8\u05E1", 61, 1],
        ["\u05E2", -1, 1],
        ["\u05D8\u05E2", 63, 4],
        ["\u05E1\u05D8\u05E2", 64, 1],
        ["\u05D9\u05E2", 63, -1],
        ["\u05E2\u05DC\u05E2", 63, 1],
        ["\u05E2\u05E0\u05E2", 63, 3],
        ["\u05D8\u05E2\u05E0\u05E2", 68, 4],
        ["\u05E2\u05E8", -1, 1],
        ["\u05D8\u05E2\u05E8", 70, 4],
        ["\u05E1\u05D8\u05E2\u05E8", 71, 1],
        ["\u05E2\u05E0\u05E2\u05E8", 70, 3],
        ["\u05D8\u05E2\u05E0\u05E2\u05E8", 73, 4],
        ["\u05D5\u05EA", -1, 32]
    ];

    /** @const */ var a_3 = [
        ["\u05D5\u05E0\u05D2", -1, 1],
        ["\u05E9\u05D0\u05E4\u05D8", -1, 1],
        ["\u05D4\u05F2\u05D8", -1, 1],
        ["\u05E7\u05F2\u05D8", -1, 1],
        ["\u05D9\u05E7\u05F2\u05D8", 3, 1],
        ["\u05DC", -1, 2]
    ];

    /** @const */ var a_4 = [
        ["\u05D9\u05D2", -1, 1],
        ["\u05D9\u05E7", -1, 1],
        ["\u05D3\u05D9\u05E7", 1, 1],
        ["\u05E0\u05D3\u05D9\u05E7", 2, 1],
        ["\u05E2\u05E0\u05D3\u05D9\u05E7", 3, 2],
        ["\u05D1\u05DC\u05D9\u05E7", 1, -1],
        ["\u05D2\u05DC\u05D9\u05E7", 1, -1],
        ["\u05E0\u05D9\u05E7", 1, 1],
        ["\u05D9\u05E9", -1, 1]
    ];

    /** @const */ var /** Array<int> */ g_niked = [255, 155, 6];

    /** @const */ var /** Array<int> */ g_vowel = [33, 2, 4, 0, 6];

    /** @const */ var /** Array<int> */ g_consonant = [239, 254, 253, 131];

    var /** number */ I_x = 0;
    var /** number */ I_p1 = 0;


    /** @return {boolean} */
    function r_prelude() {
        var /** number */ v_1 = base.cursor;
        lab0: {
            while(true)
            {
                var /** number */ v_2 = base.cursor;
                lab1: {
                    golab2: while(true)
                    {
                        var /** number */ v_3 = base.cursor;
                        lab3: {
                            lab4: {
                                var /** number */ v_4 = base.cursor;
                                lab5: {
                                    base.bra = base.cursor;
                                    if (!(base.eq_s("\u05D5\u05D5")))
                                    {
                                        break lab5;
                                    }
                                    base.ket = base.cursor;
                                    {
                                        var /** number */ v_5 = base.cursor;
                                        lab6: {
                                            if (!(base.eq_s("\u05BC")))
                                            {
                                                break lab6;
                                            }
                                            break lab5;
                                        }
                                        base.cursor = v_5;
                                    }
                                    if (!base.slice_from("\u05F0"))
                                    {
                                        return false;
                                    }
                                    break lab4;
                                }
                                base.cursor = v_4;
                                lab7: {
                                    base.bra = base.cursor;
                                    if (!(base.eq_s("\u05D5\u05D9")))
                                    {
                                        break lab7;
                                    }
                                    base.ket = base.cursor;
                                    {
                                        var /** number */ v_6 = base.cursor;
                                        lab8: {
                                            if (!(base.eq_s("\u05B4")))
                                            {
                                                break lab8;
                                            }
                                            break lab7;
                                        }
                                        base.cursor = v_6;
                                    }
                                    if (!base.slice_from("\u05F1"))
                                    {
                                        return false;
                                    }
                                    break lab4;
                                }
                                base.cursor = v_4;
                                lab9: {
                                    base.bra = base.cursor;
                                    if (!(base.eq_s("\u05D9\u05D9")))
                                    {
                                        break lab9;
                                    }
                                    base.ket = base.cursor;
                                    {
                                        var /** number */ v_7 = base.cursor;
                                        lab10: {
                                            if (!(base.eq_s("\u05B4")))
                                            {
                                                break lab10;
                                            }
                                            break lab9;
                                        }
                                        base.cursor = v_7;
                                    }
                                    if (!base.slice_from("\u05F2"))
                                    {
                                        return false;
                                    }
                                    break lab4;
                                }
                                base.cursor = v_4;
                                lab11: {
                                    base.bra = base.cursor;
                                    if (!(base.eq_s("\u05DA")))
                                    {
                                        break lab11;
                                    }
                                    base.ket = base.cursor;
                                    if (!base.slice_from("\u05DB"))
                                    {
                                        return false;
                                    }
                                    break lab4;
                                }
                                base.cursor = v_4;
                                lab12: {
                                    base.bra = base.cursor;
                                    if (!(base.eq_s("\u05DD")))
                                    {
                                        break lab12;
                                    }
                                    base.ket = base.cursor;
                                    if (!base.slice_from("\u05DE"))
                                    {
                                        return false;
                                    }
                                    break lab4;
                                }
                                base.cursor = v_4;
                                lab13: {
                                    base.bra = base.cursor;
                                    if (!(base.eq_s("\u05DF")))
                                    {
                                        break lab13;
                                    }
                                    base.ket = base.cursor;
                                    if (!base.slice_from("\u05E0"))
                                    {
                                        return false;
                                    }
                                    break lab4;
                                }
                                base.cursor = v_4;
                                lab14: {
                                    base.bra = base.cursor;
                                    if (!(base.eq_s("\u05E3")))
                                    {
                                        break lab14;
                                    }
                                    base.ket = base.cursor;
                                    if (!base.slice_from("\u05E4"))
                                    {
                                        return false;
                                    }
                                    break lab4;
                                }
                                base.cursor = v_4;
                                base.bra = base.cursor;
                                if (!(base.eq_s("\u05E5")))
                                {
                                    break lab3;
                                }
                                base.ket = base.cursor;
                                if (!base.slice_from("\u05E6"))
                                {
                                    return false;
                                }
                            }
                            base.cursor = v_3;
                            break golab2;
                        }
                        base.cursor = v_3;
                        if (base.cursor >= base.limit)
                        {
                            break lab1;
                        }
                        base.cursor++;
                    }
                    continue;
                }
                base.cursor = v_2;
                break;
            }
        }
        base.cursor = v_1;
        var /** number */ v_8 = base.cursor;
        lab15: {
            while(true)
            {
                var /** number */ v_9 = base.cursor;
                lab16: {
                    golab17: while(true)
                    {
                        var /** number */ v_10 = base.cursor;
                        lab18: {
                            base.bra = base.cursor;
                            if (!(base.in_grouping(g_niked, 1456, 1474)))
                            {
                                break lab18;
                            }
                            base.ket = base.cursor;
                            if (!base.slice_del())
                            {
                                return false;
                            }
                            base.cursor = v_10;
                            break golab17;
                        }
                        base.cursor = v_10;
                        if (base.cursor >= base.limit)
                        {
                            break lab16;
                        }
                        base.cursor++;
                    }
                    continue;
                }
                base.cursor = v_9;
                break;
            }
        }
        base.cursor = v_8;
        return true;
    };

    /** @return {boolean} */
    function r_mark_regions() {
        I_p1 = base.limit;
        var /** number */ v_1 = base.cursor;
        lab0: {
            base.bra = base.cursor;
            if (!(base.eq_s("\u05D2\u05E2")))
            {
                base.cursor = v_1;
                break lab0;
            }
            base.ket = base.cursor;
            if (!base.slice_from("GE"))
            {
                return false;
            }
        }
        var /** number */ v_2 = base.cursor;
        lab1: {
            if (base.find_among(a_0) == 0)
            {
                base.cursor = v_2;
                break lab1;
            }
            lab2: {
                var /** number */ v_3 = base.cursor;
                lab3: {
                    lab4: {
                        var /** number */ v_4 = base.cursor;
                        lab5: {
                            if (!(base.eq_s("\u05E6\u05D5\u05D2\u05E0")))
                            {
                                break lab5;
                            }
                            break lab4;
                        }
                        base.cursor = v_4;
                        lab6: {
                            if (!(base.eq_s("\u05E6\u05D5\u05E7\u05D8")))
                            {
                                break lab6;
                            }
                            break lab4;
                        }
                        base.cursor = v_4;
                        if (!(base.eq_s("\u05E6\u05D5\u05E7\u05E0")))
                        {
                            break lab3;
                        }
                    }
                    {
                        var /** number */ v_5 = base.cursor;
                        lab7: {
                            {
                                var /** number */ c1 = base.cursor + 1;
                                if (0 > c1 || c1 > base.limit)
                                {
                                    break lab7;
                                }
                                base.cursor = c1;
                            }
                            break lab3;
                        }
                        base.cursor = v_5;
                    }
                    {
                        var /** number */ c2 = base.cursor + -4;
                        if (0 > c2 || c2 > base.limit)
                        {
                            break lab3;
                        }
                        base.cursor = c2;
                    }
                    break lab2;
                }
                base.cursor = v_3;
                lab8: {
                    base.bra = base.cursor;
                    if (!(base.eq_s("\u05D2\u05E2")))
                    {
                        break lab8;
                    }
                    base.ket = base.cursor;
                    if (!base.slice_from("GE"))
                    {
                        return false;
                    }
                    break lab2;
                }
                base.cursor = v_3;
                base.bra = base.cursor;
                if (!(base.eq_s("\u05E6\u05D5")))
                {
                    base.cursor = v_2;
                    break lab1;
                }
                base.ket = base.cursor;
                if (!base.slice_from("TSU"))
                {
                    return false;
                }
            }
        }
        var /** number */ v_6 = base.cursor;
        {
            var /** number */ c3 = base.cursor + 3;
            if (0 > c3 || c3 > base.limit)
            {
                return false;
            }
            base.cursor = c3;
        }
        I_x = base.cursor;
        base.cursor = v_6;
        var /** number */ v_7 = base.cursor;
        lab9: {
            if (base.find_among(a_1) == 0)
            {
                base.cursor = v_7;
                break lab9;
            }
        }
        {
            var /** number */ v_8 = base.cursor;
            lab10: {
                if (!(base.in_grouping(g_consonant, 1489, 1520)))
                {
                    break lab10;
                }
                if (!(base.in_grouping(g_consonant, 1489, 1520)))
                {
                    break lab10;
                }
                if (!(base.in_grouping(g_consonant, 1489, 1520)))
                {
                    break lab10;
                }
                I_p1 = base.cursor;
                return false;
            }
            base.cursor = v_8;
        }
        golab11: while(true)
        {
            var /** number */ v_9 = base.cursor;
            lab12: {
                if (!(base.in_grouping(g_vowel, 1488, 1522)))
                {
                    break lab12;
                }
                base.cursor = v_9;
                break golab11;
            }
            base.cursor = v_9;
            if (base.cursor >= base.limit)
            {
                return false;
            }
            base.cursor++;
        }
        while(true)
        {
            lab13: {
                if (!(base.in_grouping(g_vowel, 1488, 1522)))
                {
                    break lab13;
                }
                continue;
            }
            break;
        }
        I_p1 = base.cursor;
        lab14: {
            if (!(I_p1 < I_x))
            {
                break lab14;
            }
            I_p1 = I_x;
        }
        return true;
    };

    /** @return {boolean} */
    function r_R1() {
        if (!(I_p1 <= base.cursor))
        {
            return false;
        }
        return true;
    };

    /** @return {boolean} */
    function r_standard_suffix() {
        var /** number */ among_var;
        var /** number */ v_1 = base.limit - base.cursor;
        lab0: {
            base.ket = base.cursor;
            among_var = base.find_among_b(a_2);
            if (among_var == 0)
            {
                break lab0;
            }
            base.bra = base.cursor;
            switch (among_var) {
                case 1:
                    if (!r_R1())
                    {
                        break lab0;
                    }
                    if (!base.slice_del())
                    {
                        return false;
                    }
                    break;
                case 2:
                    if (!r_R1())
                    {
                        break lab0;
                    }
                    if (!base.slice_from("\u05D9\u05E2"))
                    {
                        return false;
                    }
                    break;
                case 3:
                    if (!r_R1())
                    {
                        break lab0;
                    }
                    if (!base.slice_del())
                    {
                        return false;
                    }
                    {
                        var /** number */ v_2 = base.limit - base.cursor;
                        lab1: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05D2\u05D0\u05E0\u05D2")))
                            {
                                break lab1;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05D2\u05F2"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_2;
                    }
                    {
                        var /** number */ v_3 = base.limit - base.cursor;
                        lab2: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05E0\u05D5\u05DE")))
                            {
                                break lab2;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05E0\u05E2\u05DE"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_3;
                    }
                    {
                        var /** number */ v_4 = base.limit - base.cursor;
                        lab3: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05DE\u05D9\u05D8")))
                            {
                                break lab3;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05DE\u05F2\u05D3"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_4;
                    }
                    {
                        var /** number */ v_5 = base.limit - base.cursor;
                        lab4: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05D1\u05D9\u05D8")))
                            {
                                break lab4;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05D1\u05F2\u05D8"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_5;
                    }
                    {
                        var /** number */ v_6 = base.limit - base.cursor;
                        lab5: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05D1\u05D9\u05E1")))
                            {
                                break lab5;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05D1\u05F2\u05E1"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_6;
                    }
                    {
                        var /** number */ v_7 = base.limit - base.cursor;
                        lab6: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05F0\u05D9\u05D6")))
                            {
                                break lab6;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05F0\u05F2\u05D6"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_7;
                    }
                    {
                        var /** number */ v_8 = base.limit - base.cursor;
                        lab7: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05D8\u05E8\u05D9\u05D1")))
                            {
                                break lab7;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05D8\u05E8\u05F2\u05D1"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_8;
                    }
                    {
                        var /** number */ v_9 = base.limit - base.cursor;
                        lab8: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05DC\u05D9\u05D8")))
                            {
                                break lab8;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05DC\u05F2\u05D8"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_9;
                    }
                    {
                        var /** number */ v_10 = base.limit - base.cursor;
                        lab9: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05E7\u05DC\u05D9\u05D1")))
                            {
                                break lab9;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05E7\u05DC\u05F2\u05D1"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_10;
                    }
                    {
                        var /** number */ v_11 = base.limit - base.cursor;
                        lab10: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05E8\u05D9\u05D1")))
                            {
                                break lab10;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05E8\u05F2\u05D1"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_11;
                    }
                    {
                        var /** number */ v_12 = base.limit - base.cursor;
                        lab11: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05E8\u05D9\u05E1")))
                            {
                                break lab11;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05E8\u05F2\u05E1"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_12;
                    }
                    {
                        var /** number */ v_13 = base.limit - base.cursor;
                        lab12: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05E9\u05F0\u05D9\u05D2")))
                            {
                                break lab12;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05E9\u05F0\u05F2\u05D2"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_13;
                    }
                    {
                        var /** number */ v_14 = base.limit - base.cursor;
                        lab13: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05E9\u05DE\u05D9\u05E1")))
                            {
                                break lab13;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05E9\u05DE\u05F2\u05E1"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_14;
                    }
                    {
                        var /** number */ v_15 = base.limit - base.cursor;
                        lab14: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05E9\u05E0\u05D9\u05D8")))
                            {
                                break lab14;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05E9\u05E0\u05F2\u05D3"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_15;
                    }
                    {
                        var /** number */ v_16 = base.limit - base.cursor;
                        lab15: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05E9\u05E8\u05D9\u05D1")))
                            {
                                break lab15;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05E9\u05E8\u05F2\u05D1"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_16;
                    }
                    {
                        var /** number */ v_17 = base.limit - base.cursor;
                        lab16: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05D1\u05D5\u05E0\u05D3")))
                            {
                                break lab16;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05D1\u05D9\u05E0\u05D3"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_17;
                    }
                    {
                        var /** number */ v_18 = base.limit - base.cursor;
                        lab17: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05F0\u05D5\u05D8\u05E9")))
                            {
                                break lab17;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05F0\u05D9\u05D8\u05E9"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_18;
                    }
                    {
                        var /** number */ v_19 = base.limit - base.cursor;
                        lab18: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05D6\u05D5\u05E0\u05D2")))
                            {
                                break lab18;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05D6\u05D9\u05E0\u05D2"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_19;
                    }
                    {
                        var /** number */ v_20 = base.limit - base.cursor;
                        lab19: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05D8\u05E8\u05D5\u05E0\u05E7")))
                            {
                                break lab19;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05D8\u05E8\u05D9\u05E0\u05E7"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_20;
                    }
                    {
                        var /** number */ v_21 = base.limit - base.cursor;
                        lab20: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05E6\u05F0\u05D5\u05E0\u05D2")))
                            {
                                break lab20;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05E6\u05F0\u05D9\u05E0\u05D2"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_21;
                    }
                    {
                        var /** number */ v_22 = base.limit - base.cursor;
                        lab21: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05E9\u05DC\u05D5\u05E0\u05D2")))
                            {
                                break lab21;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05E9\u05DC\u05D9\u05E0\u05D2"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_22;
                    }
                    {
                        var /** number */ v_23 = base.limit - base.cursor;
                        lab22: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05D1\u05F1\u05D2")))
                            {
                                break lab22;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05D1\u05F2\u05D2"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_23;
                    }
                    {
                        var /** number */ v_24 = base.limit - base.cursor;
                        lab23: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05D4\u05F1\u05D1")))
                            {
                                break lab23;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05D4\u05F2\u05D1"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_24;
                    }
                    {
                        var /** number */ v_25 = base.limit - base.cursor;
                        lab24: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05E4\u05D0\u05E8\u05DC\u05F1\u05E8")))
                            {
                                break lab24;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05E4\u05D0\u05E8\u05DC\u05D9\u05E8"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_25;
                    }
                    {
                        var /** number */ v_26 = base.limit - base.cursor;
                        lab25: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05E9\u05D8\u05D0\u05E0")))
                            {
                                break lab25;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05E9\u05D8\u05F2"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_26;
                    }
                    {
                        var /** number */ v_27 = base.limit - base.cursor;
                        lab26: {
                            base.ket = base.cursor;
                            if (!(base.eq_s_b("\u05E9\u05F0\u05F1\u05E8")))
                            {
                                break lab26;
                            }
                            base.bra = base.cursor;
                            if (!base.slice_from("\u05E9\u05F0\u05E2\u05E8"))
                            {
                                return false;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_27;
                    }
                    break;
                case 4:
                    {
                        var /** number */ v_28 = base.limit - base.cursor;
                        lab27: {
                            if (!r_R1())
                            {
                                break lab27;
                            }
                            if (!base.slice_del())
                            {
                                return false;
                            }
                            {
                                var /** number */ v_29 = base.limit - base.cursor;
                                lab28: {
                                    base.ket = base.cursor;
                                    lab29: {
                                        var /** number */ v_30 = base.limit - base.cursor;
                                        lab30: {
                                            if (!(base.eq_s_b("\u05D2\u05E2\u05D1\u05E8\u05D0\u05DB")))
                                            {
                                                break lab30;
                                            }
                                            break lab29;
                                        }
                                        base.cursor = base.limit - v_30;
                                        if (!(base.eq_s_b("\u05D1\u05E8\u05D0\u05DB")))
                                        {
                                            break lab28;
                                        }
                                    }
                                    base.bra = base.cursor;
                                    if (!base.slice_from("\u05D1\u05E8\u05E2\u05E0\u05D2"))
                                    {
                                        return false;
                                    }
                                    break lab27;
                                }
                                base.cursor = base.limit - v_29;
                            }
                            break lab0;
                        }
                        base.cursor = base.limit - v_28;
                    }
                    while(true)
                    {
                        lab31: {
                            {
                                var /** number */ c1 = base.cursor - -1;
                                if (base.limit_backward > c1 || c1 > base.limit)
                                {
                                    break lab31;
                                }
                                base.cursor = c1;
                            }
                            continue;
                        }
                        break;
                    }
                    base.ket = base.cursor;
                    lab32: {
                        var /** number */ v_32 = base.limit - base.cursor;
                        lab33: {
                            if (!(base.eq_s_b("\u05E2\u05E0\u05E2\u05E8")))
                            {
                                break lab33;
                            }
                            break lab32;
                        }
                        base.cursor = base.limit - v_32;
                        lab34: {
                            if (!(base.eq_s_b("\u05E2\u05E0\u05E2\u05DE")))
                            {
                                break lab34;
                            }
                            break lab32;
                        }
                        base.cursor = base.limit - v_32;
                        lab35: {
                            if (!(base.eq_s_b("\u05E2\u05E0\u05E2")))
                            {
                                break lab35;
                            }
                            break lab32;
                        }
                        base.cursor = base.limit - v_32;
                        lab36: {
                            if (!(base.eq_s_b("\u05E0")))
                            {
                                break lab36;
                            }
                            break lab32;
                        }
                        base.cursor = base.limit - v_32;
                        lab37: {
                            if (!(base.eq_s_b("\u05E2\u05E8")))
                            {
                                break lab37;
                            }
                            break lab32;
                        }
                        base.cursor = base.limit - v_32;
                        lab38: {
                            if (!(base.eq_s_b("\u05E0\u05E1")))
                            {
                                break lab38;
                            }
                            break lab32;
                        }
                        base.cursor = base.limit - v_32;
                        lab39: {
                            if (!(base.eq_s_b("\u05E2")))
                            {
                                break lab39;
                            }
                            break lab32;
                        }
                        base.cursor = base.limit - v_32;
                        if (!(base.eq_s_b("\u05E1")))
                        {
                            break lab0;
                        }
                    }
                    base.bra = base.cursor;
                    if (!r_R1())
                    {
                        break lab0;
                    }
                    if (!base.slice_del())
                    {
                        return false;
                    }
                    break;
                case 5:
                    if (!base.slice_from("\u05D2\u05F2"))
                    {
                        return false;
                    }
                    break;
                case 6:
                    if (!base.slice_from("\u05E0\u05E2\u05DE"))
                    {
                        return false;
                    }
                    break;
                case 7:
                    if (!base.slice_from("\u05E9\u05E8\u05F2\u05D1"))
                    {
                        return false;
                    }
                    break;
                case 8:
                    if (!base.slice_from("\u05DE\u05F2\u05D3"))
                    {
                        return false;
                    }
                    break;
                case 9:
                    if (!base.slice_from("\u05D1\u05F2\u05D8"))
                    {
                        return false;
                    }
                    break;
                case 10:
                    if (!base.slice_from("\u05D1\u05F2\u05E1"))
                    {
                        return false;
                    }
                    break;
                case 11:
                    if (!base.slice_from("\u05F0\u05F2\u05D6"))
                    {
                        return false;
                    }
                    break;
                case 12:
                    if (!base.slice_from("\u05D8\u05E8\u05F2\u05D1"))
                    {
                        return false;
                    }
                    break;
                case 13:
                    if (!base.slice_from("\u05DC\u05F2\u05D8"))
                    {
                        return false;
                    }
                    break;
                case 14:
                    if (!base.slice_from("\u05E7\u05DC\u05F2\u05D1"))
                    {
                        return false;
                    }
                    break;
                case 15:
                    if (!base.slice_from("\u05E8\u05F2\u05D1"))
                    {
                        return false;
                    }
                    break;
                case 16:
                    if (!base.slice_from("\u05E8\u05F2\u05E1"))
                    {
                        return false;
                    }
                    break;
                case 17:
                    if (!base.slice_from("\u05E9\u05F0\u05F2\u05D2"))
                    {
                        return false;
                    }
                    break;
                case 18:
                    if (!base.slice_from("\u05E9\u05DE\u05F2\u05E1"))
                    {
                        return false;
                    }
                    break;
                case 19:
                    if (!base.slice_from("\u05E9\u05E0\u05F2\u05D3"))
                    {
                        return false;
                    }
                    break;
                case 20:
                    if (!base.slice_from("\u05D1\u05D9\u05E0\u05D3"))
                    {
                        return false;
                    }
                    break;
                case 21:
                    if (!base.slice_from("\u05F0\u05D9\u05D8\u05E9"))
                    {
                        return false;
                    }
                    break;
                case 22:
                    if (!base.slice_from("\u05D6\u05D9\u05E0\u05D2"))
                    {
                        return false;
                    }
                    break;
                case 23:
                    if (!base.slice_from("\u05D8\u05E8\u05D9\u05E0\u05E7"))
                    {
                        return false;
                    }
                    break;
                case 24:
                    if (!base.slice_from("\u05E6\u05F0\u05D9\u05E0\u05D2"))
                    {
                        return false;
                    }
                    break;
                case 25:
                    if (!base.slice_from("\u05E9\u05DC\u05D9\u05E0\u05D2"))
                    {
                        return false;
                    }
                    break;
                case 26:
                    if (!base.slice_from("\u05D1\u05F2\u05D2"))
                    {
                        return false;
                    }
                    break;
                case 27:
                    if (!base.slice_from("\u05D4\u05F2\u05D1"))
                    {
                        return false;
                    }
                    break;
                case 28:
                    if (!base.slice_from("\u05E4\u05D0\u05E8\u05DC\u05D9\u05E8"))
                    {
                        return false;
                    }
                    break;
                case 29:
                    if (!base.slice_from("\u05E9\u05D8\u05F2"))
                    {
                        return false;
                    }
                    break;
                case 30:
                    if (!base.slice_from("\u05E9\u05F0\u05E2\u05E8"))
                    {
                        return false;
                    }
                    break;
                case 31:
                    if (!base.slice_from("\u05D1\u05E8\u05E2\u05E0\u05D2"))
                    {
                        return false;
                    }
                    break;
                case 32:
                    if (!r_R1())
                    {
                        break lab0;
                    }
                    if (!base.slice_from("\u05D4"))
                    {
                        return false;
                    }
                    break;
                case 33:
                    {
                        var /** number */ c2 = base.cursor - -4;
                        if (base.limit_backward > c2 || c2 > base.limit)
                        {
                            break lab0;
                        }
                        base.cursor = c2;
                    }
                    base.ket = base.cursor;
                    if (!(base.eq_s_b("\u05D8")))
                    {
                        break lab0;
                    }
                    base.bra = base.cursor;
                    if (!r_R1())
                    {
                        break lab0;
                    }
                    if (!base.slice_del())
                    {
                        return false;
                    }
                    break;
            }
        }
        base.cursor = base.limit - v_1;
        var /** number */ v_33 = base.limit - base.cursor;
        lab40: {
            base.ket = base.cursor;
            among_var = base.find_among_b(a_3);
            if (among_var == 0)
            {
                break lab40;
            }
            base.bra = base.cursor;
            switch (among_var) {
                case 1:
                    if (!r_R1())
                    {
                        break lab40;
                    }
                    if (!base.slice_del())
                    {
                        return false;
                    }
                    break;
                case 2:
                    if (!r_R1())
                    {
                        break lab40;
                    }
                    if (!(base.in_grouping_b(g_consonant, 1489, 1520)))
                    {
                        break lab40;
                    }
                    if (!base.slice_del())
                    {
                        return false;
                    }
                    break;
            }
        }
        base.cursor = base.limit - v_33;
        var /** number */ v_34 = base.limit - base.cursor;
        lab41: {
            base.ket = base.cursor;
            among_var = base.find_among_b(a_4);
            if (among_var == 0)
            {
                break lab41;
            }
            base.bra = base.cursor;
            switch (among_var) {
                case 1:
                    if (!r_R1())
                    {
                        break lab41;
                    }
                    if (!base.slice_del())
                    {
                        return false;
                    }
                    break;
                case 2:
                    if (!r_R1())
                    {
                        break lab41;
                    }
                    {
                        var /** number */ v_35 = base.limit - base.cursor;
                        lab42: {
                            var /** number */ v_36 = base.limit - base.cursor;
                            lab43: {
                                var /** number */ v_37 = base.limit - base.cursor;
                                lab44: {
                                    if (!(base.eq_s_b("\u05E0\u05D2")))
                                    {
                                        break lab44;
                                    }
                                    break lab43;
                                }
                                base.cursor = base.limit - v_37;
                                lab45: {
                                    if (!(base.eq_s_b("\u05E0\u05E7")))
                                    {
                                        break lab45;
                                    }
                                    break lab43;
                                }
                                base.cursor = base.limit - v_37;
                                lab46: {
                                    if (!(base.eq_s_b("\u05E0")))
                                    {
                                        break lab46;
                                    }
                                    break lab43;
                                }
                                base.cursor = base.limit - v_37;
                                lab47: {
                                    if (!(base.eq_s_b("\u05DE")))
                                    {
                                        break lab47;
                                    }
                                    break lab43;
                                }
                                base.cursor = base.limit - v_37;
                                lab48: {
                                    if (!(base.eq_s_b("Lamed")))
                                    {
                                        break lab48;
                                    }
                                    if (!(base.in_grouping_b(g_consonant, 1489, 1520)))
                                    {
                                        break lab48;
                                    }
                                    break lab43;
                                }
                                base.cursor = base.limit - v_37;
                                if (!(base.in_grouping_b(g_vowel, 1488, 1522)))
                                {
                                    break lab42;
                                }
                            }
                            base.cursor = base.limit - v_36;
                            if (!base.slice_del())
                            {
                                return false;
                            }
                            break lab41;
                        }
                        base.cursor = base.limit - v_35;
                    }
                    {
                        var /** number */ c3 = base.cursor - -5;
                        if (base.limit_backward > c3 || c3 > base.limit)
                        {
                            break lab41;
                        }
                        base.cursor = c3;
                    }
                    if (!(base.eq_s_b("\u05E0\u05D3\u05D9\u05E7")))
                    {
                        break lab41;
                    }
                    if (!base.slice_del())
                    {
                        return false;
                    }
                    break;
            }
        }
        base.cursor = base.limit - v_34;
        var /** number */ v_38 = base.limit - base.cursor;
        lab49: {
            while(true)
            {
                var /** number */ v_39 = base.limit - base.cursor;
                lab50: {
                    golab51: while(true)
                    {
                        var /** number */ v_40 = base.limit - base.cursor;
                        lab52: {
                            base.ket = base.cursor;
                            lab53: {
                                var /** number */ v_41 = base.limit - base.cursor;
                                lab54: {
                                    if (!(base.eq_s_b("GE")))
                                    {
                                        break lab54;
                                    }
                                    break lab53;
                                }
                                base.cursor = base.limit - v_41;
                                if (!(base.eq_s_b("TSU")))
                                {
                                    break lab52;
                                }
                            }
                            base.bra = base.cursor;
                            if (!base.slice_del())
                            {
                                return false;
                            }
                            base.cursor = base.limit - v_40;
                            break golab51;
                        }
                        base.cursor = base.limit - v_40;
                        if (base.cursor <= base.limit_backward)
                        {
                            break lab50;
                        }
                        base.cursor--;
                    }
                    continue;
                }
                base.cursor = base.limit - v_39;
                break;
            }
        }
        base.cursor = base.limit - v_38;
        return true;
    };

    this.stem = /** @return {boolean} */ function() {
        r_prelude();
        var /** number */ v_2 = base.cursor;
        r_mark_regions();
        base.cursor = v_2;
        base.limit_backward = base.cursor; base.cursor = base.limit;
        r_standard_suffix();
        base.cursor = base.limit_backward;
        return true;
    };

    /**@return{string}*/
    this['stemWord'] = function(/**string*/word) {
        base.setCurrent(word);
        this.stem();
        return base.getCurrent();
    };
};
window['YiddishStemmer'] = YiddishStemmer;
