...
{
    fprintf(outfile, "%s", str_data(str));
}
...
{
    write_string(g, "\\u00");
    write_hexdigit(g, ch >> 4);
    write_hexdigit(g, ch & 15);
}

static void write_literal_string(struct generator * g, byte * p)
{
    int i;
    write_string(g, "\"");
    for (i = 0; i < SIZE(p); i++) {
			     ...
			     struct str * savevar)
{
    ...
    write_str(g, savevar);
    write_string(g, " = ");
    if (p->mode != m_forward) write_string(g, "limit - ");
...
static void wk(struct generator * g, struct node * p)     /* keep c */
{
    struct str * varname;
    varname = str_new();
    if (p->mode == m_forward) str_assign(varname, "c");
    else str_assign(varname, "m");

    write_savecursor(g, p, varname);
}

static void wr(struct generator * g, struct node * p)
{
    /* restore c */
    struct str * temp = str_new();
...
static void write_failure(struct generator * g)
{
	...
	write_string(g, " /*failure*/");
    }
    switch (g->failure_label)
    {
	case x_return:
	    write_string(g, "return false;"); break;
	default:
	    write_string(g, "break lab"); write_int(g, g->failure_label); write_string(g, ";");
    }
    write_string(g, " }");
}
...
{
    if (p->mode == m_forward) {
	write_string(g, "if (cursor >= limit) ");
    } else {
	write_string(g, "if (cursor <= limit_backward) ");
    }
    write_failure(g);
}
...
static void generate_loop(struct generator * g, struct node * p)
{
    ...
    write_comment(g, p);
    w(g, "~{");
    g->S[0] = str_data(loopvar);
    w(g, "~Mint ~S0;~N");
    w(g, "~Mfor (~S0 = ");
    generate_AE(g, p->AE);
    g->S[0] = str_data(loopvar);
    writef(g, "; ~S0 > 0; ~S0--)~N", p);
    writef(g, "~{", p);
...
static void generate_tomark(struct generator * g, struct node * p)
{
    ...
    w(g, "~Mif (cursor ~S0 "); generate_AE(g, p->AE); w(g, ") ~f~N");
...
{
    write_comment(g, p);
    w(g, "~Mif (cursor != "); generate_AE(g, p->AE); writef(g, ") ~f~N", p);
}
...
static void generate_atlimit(struct generator * g, struct node * p)
{
    ...
    writef(g, "~Mif (cursor ~S1 ~S0) ~f~N", p);
}
...
static void generate_setlimit(struct generator * g, struct node * p)
{
    ...
    write_block_start(g);
    write_savecursor(g, p, savevar);
    g->S[0] = str_data(varname);
    writef(g, "~Mint ~S0;~N", p);
    generate(g, p->left);
    g->S[0] = str_data(varname);
    if (p->mode == m_forward) {
	w(g, "~M~S0 = limit - cursor;~N");
	w(g, "~Mlimit = cursor;~N");
    } else {
	w(g, "~M~S0 = limit_backward;~N");
...
static void generate_grouping(struct generator * g, struct node * p, int complement)
{
    ...
    g->I[1] = q->largest_ch;
    if (q->no_gaps)
        w(g, "~Mif (!(~S1_range~S0(~I0, ~I1))) ~f~N");
    else
        w(g, "~Mif (!(~S1_grouping~S0(~V0, ~I0, ~I1))) ~f~N");
}
...
static void generate_substring(struct generator * g, struct node * p)
{
    ...
    if (x->command_count == 0 && x->starter == 0)
        writef(g, "~Mif (find_among~S0(a_~I0, ~I1) == 0) ~N"
	       "~+~M~f~N~-", p);
    else
        writef(g, "~Mamong_var = find_among~S0(a_~I0, ~I1);~N"
              "~Mif (among_var == 0) ~f~N", p);
}
...
{
    w(g, "// This file was generated automatically by the snowball to Java converter~N");
    w(g, "~N");
}
